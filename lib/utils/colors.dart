import 'dart:ui';

Color mainColor = const Color(0xFFC1C1C1);
class MyColor {
  static Color mainColor = const Color(0xFFFFFFFF);
  static const Color accentColor = Color(0xFF3F5C85);
  static const Color accentColor2 = Color(0xFF2C538C);
  static const Color iconsColor = Color(0xFF46FFB1);
  static const Color textColor = Color(0xFF000000);
  static const Color deepBlack = Color(0xFF303030);
  static const Color lightBlack = Color(0xFF495A73);
  static const Color white = Color(0xFFFFFFFF);
  static const Color darkWhite = Color(0xFFC4C4C4);

}